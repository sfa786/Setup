...
function Install-AppWithWingetSilent {
    ...
    if ($isInstalled -like "No *") {
        ...
        $StatusLabel.Refresh()
        $arguments = "install $AppName --accept-source-agreements --accept-package-agreements --silent --no-progress"

        Start-Process winget -ArgumentList $arguments -Wait
        $StatusLabel.Text = "$AppName has been installed!"
        
        
    }
    else {
        $StatusLabel.Text = "$AppName is already installed on this system."
    }
}
...
# Create a label control to display the installation status
...
$installButton.Add_Click({
    ...
    foreach ($app in $selectedApps) {
        Install-AppWithWingetSilent -AppName $app.Text -StatusLabel $statusLabel
    }
})
